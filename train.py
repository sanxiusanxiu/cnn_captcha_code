...
def evaluate_model(model, eval_loader):
    ...
    with torch.no_grad():
        for i, (images, labels) in enumerate(eval_loader):
            ...
            # 前向传播：图片 → 模型 → 144 维预测向量，因为 batch_size=1，所以 outputs 形状为 (1, 144)
            outputs = model(images)

            # 解码预测结果
            # 把 144 维向量(batch=1, 144) reshape 成 (batch=1, 4个字符, 36种字符)
            outputs = outputs.view(-1, MAX_CAPTCHA, ALL_CHAR_LIST_LEN)
            ...
            labels = labels.view(-1, MAX_CAPTCHA, ALL_CHAR_LIST_LEN)
...
def train():
    # 第1步：准备数据
    train_loader = get_train_data_loader(batch_size=train_batch_size)
    eval_loader = get_eval_data_loader(batch_size=eval_batch_size)
    # 第2步：初始化模型
